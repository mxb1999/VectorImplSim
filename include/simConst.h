...
const double intensity = 1.0e17;
const float courant_mult = 0.01; // 0.37 // 0.25 // 0.36 // 0.22;
const double uray_mult = intensity*(courant_mult)*pow(double(rays_per_zone),-1.0);
const double offset = 0;//0.5e-4;
...
const int nt=int(pow(courant_mult,-1.0)*fmax(nx,nz)*2.0)+1;
const int numstored = nx*6;// nbeams*rays_per_zone;
